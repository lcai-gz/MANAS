...
parser.add_argument('--batch_size', type=int, default=16, help='batach size')  ##
...
parser.add_argument('--ckt_path', type=str, default='search-EXP-64_three_cells_training_loss-steps2-half-batch16-ssim-20220718-081254/best_ssim_weights.pt', help='checkpoint path of search')
args = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

args.save = 'eval-{}-{}-{}'.format(args.save, 'three-cells-training-loss-steps2-half-batch16-ssim', time.strftime("%Y%m%d-%H%M%S"))  ##
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for steps in range(1000):  ##
            ...
            optimizer.zero_grad()
            output = model(input)
            loss = Muti_loss_ssim(output, target, 16, epoch)  ##
            loss.backward()
            # nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
            optimizer.step()
            training_loss += loss.item()
            logging.info('training_loss:%6f', loss)

        epoch_loss = training_loss / 1000
        training_loss_list.append(epoch_loss)
        avg_training_loss = np.mean(training_loss_list)
        ...
        if loss < best_loss:
            ...
            best_loss = loss
        scheduler.step()
        logging.info('psnr:%6f ssim:%6f loss:%6f epoch_loss:%6f avg_training_loss:%6f -- best_psnr:%6f best_ssim:%6f best_loss:%6f', psnr, ssim, loss, epoch_loss, avg_training_loss, best_psnr, best_ssim, best_loss)
