...
class MixedSearchSpace(nn.Module):
    ...
    ## 网络的参数初始化，auto_param表示auto attention的参数，arch_param表示multi-scale network的参数,注意是搜索的参数，不是网络权重
    ...
    def forward(self, x):
        ...
        # construct a auto_attention module, 注意x_list是多个支路的输出，因此对个每个支路运用auto_attention
        for i in range(self.auto_module[0]):  # self.auto_module[0] = 2 表示有两个支路
        ...
        for i in range(self.auto_module[1]):  # self.auto_module[1] = 3 表示有三个支路需要加auto_attention
        ...
        # construct auto_attention module
        for i in range(self.auto_module[2]):   # self.auto_module[2] = 4 表示有四个支路需要添加auto_attention
...
## 对search到super-network进行定义和forward
class FixedMixedAutoNet(nn.Module):
    def __init__(self, ckt_path, in_channels=3, list_channels=[32, 64, 128, 256], list_module=[2, 4, 4, 4], steps=2, auto_module=[2, 3, 4]):
        ...
        self.steps = steps
        self.list_modules = list_module
        arch = torch.load(ckt_path)['arch_param']  ## 加载search到multi-scale network的参数
        auto = torch.load(ckt_path)['auto_param']  ## 加载search到auto_attention的参数
        softmax = nn.Softmax(dim=1)
        arch_soft = softmax(arch)
        auto_soft = softmax(auto)
        ...
        arch_hard = torch.argmax(arch_soft, dim=1) ## 返回指定维度(dim=1表示行)最大值的序号
        auto_hard = torch.argmax(auto_soft, dim=1) ## 返回指定维度(dim=1表示行)最大值的序号
    ...
    def make_layer(self, inplanes, planes, blocks, stride=1):
        ...
        layers.append(Bottleneck(inplanes, planes, stride, downsample))  ## 有进行下采样的Bottleneck
        inplanes = planes * Bottleneck.expansion
        for _ in range(1, blocks):
            layers.append(Bottleneck(inplanes, planes))   ## 没有进行下采样的Bottleneck
        return nn.Sequential(*layers)
